  ...
  return (
    <div className="min-h-screen mesh-bg font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
        ...
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden md:flex items-center bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${activeTab === 'workbench' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
            >
            ...
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${activeTab === 'history' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
            >
              <History className="w-4 h-4" />
              History
              {history.length > 0 && (
                <span className="bg-slate-200 dark:bg-slate-600 text-slate-600 dark:text-slate-300 text-[10px] px-1.5 rounded-full">{history.length}</span>
              )}
            </button>
          </nav>
        </div>
              ...
              {data && (
                <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-1 rounded-xl mr-2">
                  <button 
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${workbenchTab === 'data' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 dark:text-slate-400'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${workbenchTab === 'insights' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 dark:text-slate-400'}`}
                  >
                    <div className="flex items-center gap-1.5">
          ...
          {activeTab === "workbench" ? (
            ...
            >
              {(!data && !rawTextData) ? (
                ...
                >
                  {/* Left Panel: Preview */}
                  <div className="lg:w-1/3 bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col min-h-[300px]">
                    <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950 flex items-center justify-between">
                      <p className="text-sm font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">Document Source{files.length > 1 ? 's' : ''}</p>
                    </div>
                  ...
                  <div className="lg:w-2/3 bg-white dark:bg-slate-900 rounded-3xl shadow-lg border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col min-h-[500px]">
                    {rawTextData ? (
                      <div className="flex-1 flex flex-col h-full bg-slate-50 dark:bg-slate-950 relative">
                        <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between z-10 shadow-sm">
                          <div className="flex items-center gap-2">
                            <FileText className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                            <p className="text-sm font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">Transcribed Text</p>
                          </div>
                          <div className="flex items-center gap-6 text-xs font-bold text-slate-400 uppercase tracking-widest">
                            <span className="bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">Words: {rawTextData.wordCount}</span>
                            <span className="bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">Chars: {rawTextData.charCount}</span>
                            <button
                              onClick={() => {
            ...
            >
              ...
              {filteredHistory.length > 0 ? (
                ...
                <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800">
                  ...
                  <p className="text-slate-500 dark:text-slate-400">Processed documents will appear here automatically.</p>
                </div>
              )}
            </motion.div>
          )}
      ...
      <footer className="fixed bottom-6 right-6 pointer-events-none">
        <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md px-4 py-2 rounded-full border border-slate-200 dark:border-slate-800 shadow-xl flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
          <p className="grow shrink-0 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Enterprise Processing Engine</p>
        </div>
      </footer>
    </div>
  );
